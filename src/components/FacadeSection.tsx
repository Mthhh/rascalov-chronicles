...
const FacadeSection = () => {
  ...
  return <section id="facade" ref={ref} className="relative py-32 px-6 overflow-hidden">
      {/* Background grid */}
      ...
      <div className="max-w-6xl mx-auto relative">
          ...
          <div className="bg-card/50 border border-steel/20 overflow-hidden relative group">
            ...
            <div className="absolute inset-0 overflow-hidden">
              ...
              ease: "easeOut"
                ...
                <img src={warehouseImage} alt="MM Global Export Warehouse" className="w-full h-full object-cover filter grayscale opacity-20 group-hover:opacity-30 transition-opacity duration-500" />
              </motion.div>
            ...
            <div className="relative z-10 p-8 md:flex md:gap-12">
              {/* Logo/Icon area */}
              <motion.div className="flex-shrink-0 mb-6 md:mb-0 relative" onMouseEnter={() => setIsLogoHovered(true)} onMouseLeave={() => setIsLogoHovered(false)}>
                {/* Logo container */}
                <motion.div className="relative w-32 h-32 md:w-40 md:h-40 bg-background border border-steel/20 flex items-center justify-center backdrop-blur-sm overflow-hidden" animate={{
                ...
                duration: 0.4
                  ...
                  {/* Logo image */}
                  <img src={mmGlobalLogo} alt="MM Global Export" className="w-28 h-28 md:w-36 md:h-36 object-contain" />
                </motion.div>
              </motion.div>
              ...
              <div className="flex-1">
                ...
                <p className="font-rajdhani text-foreground/70 leading-relaxed mb-6">Quand on passe de gang de rue à famille structurée, il faut une façade. MM Global Export est cette façade la couverture légale parfaite pour l'ensemble des opérations logistiques de la cellule à Los Santos. Transit de marchandises, stockage sécurisé, flotte complète de véhicules.<span className="text-ivory">gang de rue</span> à <span className="text-ivory">famille structurée</span>, 
                ...
                <p className="font-rajdhani text-foreground/60 leading-relaxed mb-4">C'est la première vraie infrastructure de la cellule. Le premier signe que ce groupe n'est plus un simple gang, mais le début de quelque chose de bien plus grand. Elle évoluera avec l'expansion comme la Rascalov elle-même.<span className="text-primary">première vraie infrastructure</span> de la cellule. 
                ...
                <div className="grid md:grid-cols-3 gap-4 mb-8">
                  {services.map((service, index) => <motion.div key={service.title} className="bg-secondary/30 border border-steel/20 p-4" initial={{
                  opacity: 0,
                  y: 20
                }} animate={isInView ? {
                  opacity: 1,
                ...
                {/* CTA Button */}
                <motion.a href="https://mm-global-export.vercel.app/" target="_blank" rel="noopener noreferrer" className="group inline-flex items-center gap-3 px-8 py-4 bg-blood/10 border border-blood/50 hover:bg-blood/20 hover:border-blood transition-all duration-300" whileHover={{
                scale: 1.02
              }} whileTap={{
                scale: 0.98
              }}>
        ...
        <motion.div className="absolute -right-4 top-1/3 hidden lg:block" initial={{
        opacity: 0,
        rotate: -5
      }} animate={isInView ? {
        opacity: 0.7,
